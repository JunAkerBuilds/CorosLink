...
async function main() {
  ...
  try {
    ...
    await until(`Boolean(document.querySelector('.calendar-workout-event select'))`, "event editor inside calendar");
    await fill(".calendar-workout-event select", "timed");
    ...
    await fill(".calendar-workout-event select", "all-day");
    await click("Save event");
    await until(`window.eventCalls.length === 3 && document.querySelector('.calendar-chip-planned').textContent.includes('All day')`, "all-day event save");
    await js(`document.querySelector('[aria-label="Close details"]').click()`);
    await until(`!document.querySelector('.calendar-workout-event')`, "closed editor");
    await js(`{
