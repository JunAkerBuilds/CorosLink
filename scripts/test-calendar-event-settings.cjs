...
async function main() {
  ...
  try {
    ...
    await fill("input[type=number]", "45");
    ...
    assert.deepEqual(await js("window.settingsCalls"), [{provider:"google",input:{eventTiming:{mode:"timed",startTime:"07:30",timeZone:"Europe/Paris",fallbackDurationMinutes:45}}}]);
    ...
    await click("Cancel");
    assert.equal(await js(`document.querySelector('input[type=number]').value`),"60");
