  ...
  return <div className="wf-simulation" ref={root}>
    ...
    {open && <section id={id} className="wf-simulation-panel" role="dialog" aria-label="Watch face simulation">
      ...
      <fieldset disabled={!simulation.enabled}>
        ...
        {Object.entries(design.nativeData ?? {}).some(([id, style]) => id !== "weather_temp" && id !== "date_year" && style.enabled) && <details><summary>Health, training & astronomy</summary><p>Sample values and asset states for the native fields on this face.</p><div className="wf-simulation-grid">
          {NATIVE_DATA_FIELDS.filter(field => field.id !== "weather_temp" && field.id !== "date_year" && design.nativeData?.[field.id]?.enabled).map(field => {
