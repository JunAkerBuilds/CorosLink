...
export const NATIVE_CHART_LINE_AVAILABILITY: NativeDataAvailability = {
  status: "unavailable",
  label: "Not available right now",
  message: "Line graphs are disabled for now. Chart previews use bars, including designs previously saved with a line sample."
};
...
export const NATIVE_DATA_FIELDS: readonly NativeDataDefinition[] = [
  ...
  { id: "weather_direction", label: "Wind direction", category: "Weather", kind: "state", sample: "2", version: 0, stateKey: "weather_direction_icon_dir", stateCount: 16 },
  ...
  number("weather_uv", "UV index", "Weather", "4", 0),
  ...
  number("stamina", "Stamina", "Health", "82", 3, { unit: "%", percentKey: "stamina_percent_icon" }),
...
export function nativeFieldKeys(field: NativeDataDefinition): string[] {
  if (field.kind === "state") return [field.stateKey!, field.id === "weather_direction" ? "weather_direction_icon_pos" : `${field.id}_pos`];
  ...
  if (field.kind === "chart") return ["chart_pos", "chart_rect", "chart_bar_width", "chart_bar_interval", "chart_selected_bar_color", "chart_unselected_bar_color", "chart_curves_upper_color", "chart_curves_lower_color", "chart_curves_width", "chart_bar_mask", "chart_bar_mask_pos", "chart_bar_nodata_mask", "chart_bar_nodata_mask_pos", "chart_point_icon", "chart_percent_icon", "chart_dgree_icon", "chart_negasign_icon", "chart_colon_icon", "chart_sunrise_hour_rect", "chart_sunrise_minute_rect", "chart_sunrise_font", "chart_sunrise_icon_pos", "chart_sunrise_icon", "chart_sunset_icon", "chart_moonrise_hour_rect", "chart_moonrise_minute_rect", "chart_moonrise_font", "chart_moonrise_icon_pos", "chart_moonrise_icon", "chart_moonset_icon", "chart_moon_icon_pos", "chart_moon_icon", "chart_sun_icon_pos", "chart_sun_icon", ...NATIVE_CHART_SOURCES.filter(({id}) => !["chart_sunrise", "chart_moonrise", "chart_moon"].includes(id)).flatMap(({id}) => [`${id}_icon_pos`, `${id}_icon`, `${id}_rect`, `${id}_font`])];
  return [`${field.id}_rect`, `${field.id}_font`, ...(!field.id.startsWith("weather_temp") ? [`${field.id}_icon_pos`, `${field.id}_icon`] : []), ...(field.percentKey ? [field.percentKey] : []), ...(field.unitKey ? [field.unitKey] : [])];
}
